...
// ------------------------
	...
	// THEN clear and draw signal traces ON TOP of grid
	clearNDrawSignals();
...
// ------------------------
	...
	if(currentFocus == L_zoom)
     	tft.drawRect(45, 0, 35, vOffset, ILI9341_WHITE); // Increased width for decimals
...
// Calculate actual visible window based on zoom and buffer
float zoomMultiplier = zoomFactor / 10.0f;
uint16_t visibleSamples = (uint16_t)(GRID_WIDTH * zoomMultiplier);
if(visibleSamples > currentBufferSize) {
    visibleSamples = currentBufferSize;
...
float xCursorRatio = (float)xCursor / currentBufferSize;
float xCursorPx = xCursorRatio * sampleSizePx + lOffset;
